...
BallShooter::BallShooter(int servoPin, int limitSwitchPin, int INA, int INB, int grb_ref1, int grb_ref2, int stepPin, int dirPin)
  : servoPin_(servoPin),
...
void BallShooter::wheel(int pwm) { // ON fly wheel
  analogWrite(INA_, pwm);
  analogWrite(INB_, 0);
}

void BallShooter::wheel_stop() { // OFF fly wheel
  analogWrite(INA_, 0);
  analogWrite(INB_, 0);
}

// additional function for control
void BallShooter::preparing(){ // keep the grabber on the ground to grab a ball
